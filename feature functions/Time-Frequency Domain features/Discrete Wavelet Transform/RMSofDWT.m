function rms_dwt_feat = RMSofDWT(x, winsize, wininc, J)
% J- level of wavelet decomposition
datasize = size(x,1);
Nchannels = size(x,2);
numwin = floor((datasize - winsize)/wininc)+1;

rms_dwt_feat = zeros(numwin, (J+1)*Nchannels);
...
for dims = 1:Nchannels
    ...
    tab_RMS = zeros(numOfSIGs, level+1);
    ...
    for k = 1:level+1
        nbCFS                = longs(k);
        en                   = st+nbCFS-1;
        tab_RMS(:,k) = rms(percentENER(:, st:en), 2);
        st = en + 1;
    end
    rms_dwt_feat(:,(1:(J+1))+(dims-1)*(J+1)) = tab_RMS;
end