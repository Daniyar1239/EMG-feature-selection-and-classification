function wl_dwt_feat = WLofDWT(x, winsize, wininc, J)
% J- level of wavelet decomposition
datasize = size(x,1);
Nchannels = size(x,2);
numwin = floor((datasize - winsize)/wininc)+1;

wl_dwt_feat = zeros(numwin, (J+1)*Nchannels);
...
for dims = 1:Nchannels
    ...
    for k = 1:level+1
        nbCFS                = longs(k);
        en                   = st+nbCFS-1;
        tab_WL(:,k) = sum(abs(diff(percentENER(:, st:en)')))';
        st = en + 1;
    end
    wl_dwt_feat(:,(1:(J+1))+(dims-1)*(J+1)) = tab_WL;
end